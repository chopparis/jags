export default async function getExchangeRates(req,res) {
    ...
    //const data = await fetch('http://192.168.6.23/api/player/registration',{ 
      ...
      let obj = { "jsonrpc": "2.0", "id": 0, method: "get_exchange_rates", params:  {site_code: process.env.NEXT_PUBLIC_SITE_CODE, "api_key": process.env.NEXT_PUBLIC_API_KEY }}

      const data = await fetch(`${process.env.NEXT_PUBLIC_EXCHANGE_RATE}`,{ 
        method: 'POST', 
